...
/**
 * SparkMax-backed indexer motor with baked-in limits, gearing, and telemetry.
 * <p>
 * All positions and velocities are reported in mechanism (roller) radians and radians per second after applying the configured gear ratio. The roller
 * uses coast mode so it does not fight deceleration when the indexer is idle.
 * </p>
 */
public class IndexerMotor extends AbstractMotor {
    ...
    private final IndexerMotorConfig config;
    ...
    public void refreshConfiguration() {
        ...
        if (motorCanId != lastMotorCanId) {
            log.warning("IndexerMotor CAN ID changed from " + lastMotorCanId + " to " + motorCanId

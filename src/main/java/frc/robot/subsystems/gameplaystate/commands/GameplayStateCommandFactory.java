 ...
 * {@link GameplayState}.
 * <p>
 * Each factory method produces a command group that sets the appropriate
 * gameplay state on the {@link GameplayStateSubsystem} and then schedules
 * mechanism commands in parallel so the robot acts as a unified system. The
 * factory delegates to individual subsystem command factories rather than
...
public class GameplayStateCommandFactory extends AbstractSubsystemCommandFactory<GameplayStateSubsystem> {
    ...
    /**
     * Creates a factory that composes gameplay state commands from individual
     * subsystem command factories.
     *
     * @param subsystem                      gameplay state subsystem that tracks
     ...
     * @param distanceToTargetMetersSupplier supplier returning the distance from
     *                                       the robot to the active scoring target
     *                                       in meters
     * @param turretReadySupplier            supplier returning true when the turret
    ...
    /**
     * Builds the IDLE state command group: all subsystems return to their resting
     * positions.
     * <p>
     * The shooter idles at its configured idle RPM, the indexer and feeder stop
     * completely, the intake stops, and the harvester stows.
     * </p>
     *
    ...
    /**
     * Builds the HARVEST_READY state command group: deploys the harvester arm and
     * spins the intake and feeder to pull Fuel into the robot while the
     * shooter idles.
     *
    ...
    /**
     * Builds the FIRE_READY state command group: spins up the shooter and stages
     * Fuel in the indexer while the turret continues tracking via its
     * default command.
     * <p>
     * Both the indexer and feeder wait until the shooter and turret report ready
     * before activating. The indexer gates on readiness via
     * {@code createFireWhenReadyCommand}, and the feeder similarly waits before
     * running its reverse-pulse-then-forward sequence. When the command
     * ends or is interrupted (e.g., operator releases the fire trigger), all
     * mechanisms transition back to IDLE so the indexer and feeder stop
     * cleanly. The turret is not explicitly commanded here because it runs its
     * default tracking command at all times.
     * </p>
     *
    ...
    /**
     * Builds the EJECT state command group: reverses all ball-path mechanisms to
     * expel Fuel.
     * <p>
     * The shooter stops, and the indexer, feeder, and intake all run in reverse to
     * push Fuel out of the robot. The harvester deploys so ejected Fuel
     * can exit cleanly.
     * </p>
     *
    ...
    /**
     * Builds the TRAVEL state command group: stows the harvester and idles all
     * other mechanisms for safe field traversal.
     * <p>
     * This state is useful when driving across the field without actively
     * collecting or scoring. The harvester moves to its stowed position so it
     * stays inside the frame perimeter, while the shooter, indexer, feeder, and
     * intake return to their idle behaviors.
     * </p>
     *
     * @return parallel command group that stows the harvester and idles all other
     *         mechanisms
     */
    ...
    /**
     * Builds the TRENCH_TRAVEL state command group: deploys the harvester for
     * low-profile clearance under field obstacles while idling all other
     * mechanisms.
     * <p>
     * This state is identical to TRAVEL except the harvester is deployed instead of
     * stowed, allowing the robot to pass under structures such as the
     * trench run.
     * </p>
     *
     * @return parallel command group that deploys the harvester and idles all other
     *         mechanisms
     */
    ...
    /**
     * Sets the idle command as the default behavior for the gameplay state
     * subsystem.
     *

 ...
 * <p>
 * Each factory method produces a command group that sets the appropriate gameplay state on the
 * {@link GameplayStateSubsystem} and then schedules mechanism commands in parallel so the robot
 * acts as a unified system. The factory delegates to individual subsystem command factories rather
 * than driving motors directly, keeping subsystem ownership intact.
 * </p>
 */
public class GameplayStateCommandFactory extends AbstractSubsystemCommandFactory<GameplayStateSubsystem> {
     ...
     * <p>
     * This is the central dispatch method. It records the state on the {@link GameplayStateSubsystem} and
     * returns the appropriate parallel command group for the requested state.
     * </p>
     *
     ...
     * <p>
     * The shooter idles at its configured idle RPM, the indexer and feeder idle, the intake stops,
     * and the harvester stows.
     * </p>
     *
    ...
    /**
     * Builds the HARVEST_READY state command group: deploys the harvester arm and spins the intake and feeder
     * to pull Fuel into the robot while the shooter idles.
     *
    ...
    /**
     * Builds the FIRE_READY state command group: spins up the shooter, aims the turret at the field target,
     * and stages Fuel in the indexer while the harvester stows.
     * <p>
     * The indexer holds until the shooter and turret report ready, then feeds automatically via
     * {@code createFireWhenReadyCommand}.
     * </p>
     *
     ...
     * <p>
     * Deploys the harvester and runs the intake to collect Fuel while the shooter spins up
     * and the turret tracks the scoring target. This allows autonomous routines to collect and
     * score without explicit state transitions between harvest and fire.
     * </p>
     *
     ...
     * <p>
     * The shooter, indexer, feeder, and intake all idle while the harvester stows to clear the climber's
     * path. The climber is commanded to its deploy position.
     * </p>
     *
     ...
     * <p>
     * The shooter stops, and the indexer, feeder, and intake all run in reverse to push Fuel out

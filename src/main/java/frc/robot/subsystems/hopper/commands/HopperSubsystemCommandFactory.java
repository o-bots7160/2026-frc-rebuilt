package frc.robot.subsystems.hopper.commands;

import java.util.function.Supplier;

import edu.wpi.first.wpilibj2.command.Command;
import edu.wpi.first.wpilibj2.command.Commands;
import frc.robot.shared.commands.AbstractVelocityCommandFactory;
import frc.robot.subsystems.hopper.HopperSubsystem;

/**
 * Factory that creates hopper commands and wires default behaviors.
 */
public class HopperSubsystemCommandFactory extends AbstractVelocityCommandFactory<HopperSubsystem> {

    /**
     * Creates a factory for commands that share the given hopper subsystem instance.
     *
     * @param subsystem hopper subsystem instance that commands created by this factory will control
     */
    public HopperSubsystemCommandFactory(HopperSubsystem subsystem) {
        super(subsystem);
    }

    /**
     * Builds an idle command that holds the belt at the configured idle RPM.
     *
     * @return command that idles the hopper belt
     */
    public IdleHopperCommand createIdleCommand() {
        return new IdleHopperCommand(subsystem);
    }

    /**
     * Builds a purge command that reads its target RPM from a supplier.
     * <p>
     * The supplier should return a negative RPM so the belt spins backward toward the intake.
     * </p>
     *
     * @param targetRpmSupplier provider for the target reverse RPM (negative value); evaluated on initialize
     * @return command that spins the belt in reverse at the supplied RPM
     */
    public PurgeHopperCommand createPurgeCommand(Supplier<Double> targetRpmSupplier) {
        return new PurgeHopperCommand(subsystem, targetRpmSupplier);
    }

    /**
     * Builds a purge command using the configured default reverse velocity.
     * <p>
     * The configured reverse RPM is negated so the belt spins backward toward the intake.
     * </p>
     *
     * @return command that spins the belt in reverse at the default purge RPM
     */
    public PurgeHopperCommand createPurgeCommand() {
        return new PurgeHopperCommand(subsystem, -subsystem.getConfig().getReverseVelocityRpm());
    }

    /**
     * Builds a command that spins the belt forward at the configured transport velocity and holds that speed indefinitely.
     * <p>
     * Use this with {@code whileTrue} so the belt transports Fuel while a button is held and returns to idle when released.
     * </p>
     *
     * @return command that transports Fuel forward and holds velocity until interrupted
     */
    public Command createForwardAndHoldCommand() {
        return Commands.runOnce(subsystem::setForwardVelocity, subsystem)
                .andThen(Commands.run(subsystem::seekVelocity, subsystem));
    }

    /**
     * Builds a command that spins the belt in reverse at the configured purge velocity and holds that speed indefinitely.
     * <p>
     * Use this with {@code whileTrue} so the belt purges Fuel while a button is held and returns to idle when released.
     * </p>
     *
     * @return command that purges Fuel backward and holds velocity until interrupted
     */
    public Command createPurgeAndHoldCommand() {
        return Commands.runOnce(subsystem::setReverseVelocity, subsystem)
                .andThen(Commands.run(subsystem::seekVelocity, subsystem));
    }

    /**
     * Builds a command that continuously reads a target RPM from a supplier and seeks that velocity every cycle.
     * <p>
     * Unlike fixed-target commands which lock the target at command start, this command re-evaluates the supplier each cycle. Use this when the
     * target is backed by a tunable value that operators may adjust while the command is running.
     * </p>
     *
     * @param targetRpmSupplier provider for the target belt RPM; evaluated every execute cycle
     * @return command that continuously tracks the supplied RPM until interrupted
     */
    public Command createContinuousTransportCommand(Supplier<Double> targetRpmSupplier) {
        return Commands.run(() -> {
            subsystem.setTargetVelocityRpm(targetRpmSupplier.get());
            subsystem.seekVelocity();
        }, subsystem);
    }

    /**
     * Builds a command that stops the hopper belt immediately.
     *
     * @return command that sets the hopper to 0 RPM and stops the motor
     */
    public Command createStopCommand() {
        return Commands.runOnce(subsystem::stop, subsystem);
    }

    /**
     * Sets the idle command as the default command for the hopper subsystem.
     *
     * @return the idle command that was set as default
     */
    public Command setDefaultIdleCommand() {
        Command command = createIdleCommand();
        subsystem.setDefaultCommand(command);
        return command;
    }
}
